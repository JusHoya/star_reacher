...
> [!IMPORTANT]
> **Status: Phase 3 (environment force models).** On top of the Phase 1 skeleton (Apache-2.0 per [ADR 0001](docs/adr/0001-license-and-visibility.md)) and the Phase 2 math kernel (time systems, frame family, DE440s ephemerides, RK4/RKF7(8) integrators, event detection), the full orbital perturbation environment is in place: Pines normalized-harmonic gravity (EGM2008/GRGM1200A/MRO120F), Battin third-body acceleration, cannonball SRP with dual-cone conical shadow, and USSA76/Harris–Priester/Mars-exponential atmospheres driving cannonball drag — composed behind `star run` via the `[environment]`/`[spacecraft]` mission schema, each with golden-vector validation, a derived math-library chapter, and frozen GMAT/Orekit cross-tool baselines (D-15). Vehicle 6DOF begins in Phase 4. Track what is actually built in the [Roadmap](#roadmap).
...
Through Phase 3: the build/install path, the `star` CLI, the SRLOG v1 log format, the RNG streams, the verification harness, and the documentation and citation machinery on all four CI platforms (Phase 1); TAI/UTC/TT/TDB time systems, the GCRF/ITRF/Moon-PA/Mars-IAU frame family, the DE440s ephemeris pipeline, RK4/RKF7(8) integrators, and event detection (Phase 2); and the full orbital perturbation environment — harmonic gravity, third-body, SRP with conical shadow, and atmospheric drag, composed behind `star run` with frozen GMAT/Orekit cross-tool baselines (Phase 3). The vehicle 6DOF (mass properties, propulsion, aero, attitude, staging) begins in Phase 4. The [Roadmap](#roadmap) reflects the true state per phase.
