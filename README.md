...
> [!IMPORTANT]
> **Status: Phase 8 (validation campaign, report, release) complete and merged to `main` at `6d8b3af`, all 18 CI checks green. Version 0.8.0 is prepared, not released.** The evidence package is closed: all seven frozen cross-tool cases measure inside their stated tolerances against GMAT and Orekit truth, both PDFs build warning-clean and rebuild byte-identically, the report case-study figures regenerate bit-for-bit under the pinned matplotlib/FreeType, and a fresh machine reaches a rendered trajectory using README commands only with `star verify` printing `VERIFY: PASS (29/29)`. Exactly two items gate the `v0.8.0` tag, and neither is a physics result: one green post-merge `nightly` run ([pre-release checklist](docs/release_checklist.md) item 3, discharged 2026-07-27 against the merged main), and the tag itself, whose `release` job builds the four-platform wheels and `verify --quick`-smokes each of them (item 11). The clauses that need a resource the maintainer does not have — Pi 5 silicon, a MATLAB license — are re-scheduled to [Phase 9](#roadmap) along with the one deferred frozen-format field, the SRLOG 1.4 `gnc.error_layout` header: four register items in all (1, 2, 4, and 9). That is a re-scheduling, not a discharge and not a waiver, so each keeps its register entry and its deferral record, the nightly `ubuntu-24.04-arm` leg stays a documented aarch64 proxy, and an aarch64 number is never a Pi 5 number.
>
