...
| 8 | **Validation campaign, report, release** — full cross-tool table, completed report, fresh-machine walkthrough, tagged release | Every cross-tool case within stated tolerance (e.g., trans-lunar < 1 km at arrival); byte-identical doc rebuilds; fresh-machine README walkthrough succeeds; release wheels pass `verify --quick` on all four platforms | Deliverables complete and the locally-closable criteria met: the scientific report and math library are complete (chapter-accretion lint green, every code-cited equation label and validation-table test ID resolves) and build warning-clean in the CI TeX Live job; the report case-study figures regenerate bit-for-bit under the pinned matplotlib/FreeType; the fresh-machine [walkthrough](docs/walkthrough.md) reaches a rendered trajectory using README commands only and `star verify` passes (29/29) in ~9 s on x86-64; the example gallery is committed. The extended cross-tool table's frozen GMAT truth (Molniya, lunar/Mars orbiters, trans-lunar, Earth–Mars cruise) and the `< 10 min on a Pi 5` timing clause are deferred, fully prepared, to the [pre-release checklist](docs/release_checklist.md) items 10 and 1 (PRD §9 valve); the tag-triggered release job builds the four-platform wheels and runs `verify --quick` on each |
