...
> [!IMPORTANT]
> **Status: Phase 4 (vehicle 6DOF).** On top of the Phase 1 skeleton (Apache-2.0 per [ADR 0001](docs/adr/0001-license-and-visibility.md)), the Phase 2 math kernel, and the Phase 3 environment force models, the launch-to-orbit-capable vehicle stack is in place: the KSP-lite vehicle TOML schema with a four-pass accumulate-all-errors validator, analytic settled-tank mass properties with the staging state remap, propulsion with throttle/gimbal limits plus RCS and reaction-wheel actuators, axisymmetric aerodynamics, and rigid-body attitude dynamics with gravity-gradient torque — each with golden-vector validation and a derived math-library chapter. A four-vehicle starter fleet flies two scripted, open-loop missions — pad-to-LEO ascent and trans-lunar injection — that each run in one command with bit-identical reruns, cross-checked against an independent 3DOF ascent. Data out (plots, HTML playback, exporters) begins in Phase 5. Track what is actually built in the [Roadmap](#roadmap).
...
Through Phase 4: the build/install path, the `star` CLI, the SRLOG log format, the RNG streams, the verification harness, and the documentation and citation machinery on all four CI platforms (Phase 1); TAI/UTC/TT/TDB time systems, the GCRF/ITRF/Moon-PA/Mars-IAU frame family, the DE440s ephemeris pipeline, RK4/RKF7(8) integrators, and event detection (Phase 2); the full orbital perturbation environment — harmonic gravity, third-body, SRP with conical shadow, and atmospheric drag, composed behind `star run` with frozen GMAT/Orekit cross-tool baselines (Phase 3); and the vehicle 6DOF — KSP-lite vehicle schema and validator, analytic mass properties, propulsion, RCS and reaction-wheel actuators, axisymmetric aero, rigid-body attitude with gravity-gradient torque, and staging with the state remap, exercised by a starter fleet flying scripted ascent and trans-lunar-injection missions and cross-checked against an independent 3DOF ascent (Phase 4). Data out — plots, HTML playback, and NPZ/Parquet exporters — begins in Phase 5. The [Roadmap](#roadmap) reflects the true state per phase.
