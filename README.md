...
> [!IMPORTANT]
> **Status: v0.8.0 released 2026-07-27, tagged at `3e111ad`.** Phase 8 (validation campaign, report, release) is complete and merged to `main`, CI is green on all four platforms, and both release-gating checklist items are discharged — the green post-merge `nightly` run (item 3) and the four-platform wheel build and `verify --quick` smoke that the tag triggered (item 11). The evidence package is closed: all seven frozen cross-tool cases measure inside their stated tolerances against GMAT and Orekit truth, both PDFs build warning-clean and rebuild byte-identically, the report case-study figures regenerate bit-for-bit under the pinned matplotlib/FreeType, and a fresh machine reaches a rendered trajectory using README commands only with `star verify` printing `VERIFY: PASS (29/29)`. Two items gated the `v0.8.0` tag and both are discharged, neither being a physics result: one green post-merge `nightly` run ([pre-release checklist](docs/release_checklist.md) item 3), and the tag's own `release` job, which built the four-platform wheels and `verify --quick`-smoked each of them in an isolated venv (item 11). The clauses that need a resource the maintainer does not have — Pi 5 silicon, a MATLAB license — are re-scheduled to [Phase 9](#roadmap) along with the one deferred frozen-format field, the SRLOG 1.4 `gnc.error_layout` header: four register items in all (1, 2, 4, and 9). That is a re-scheduling, not a discharge and not a waiver, so each keeps its register entry and its deferral record, the nightly `ubuntu-24.04-arm` leg stays a documented aarch64 proxy, and an aarch64 number is never a Pi 5 number.
>
> **Phase 7 (batch, Monte Carlo, ML layer).** The AI/ML training and evaluation surface is in: `star mc` expands a TOML sweep spec (grid, list, or Latin hypercube) into an N-worker process pool with per-run seed = SplitMix64(master, index) and a `manifest.json` that makes every run individually reproducible — a 256-run/8-worker LHS sweep finishes 256/256 `success` and any entry re-runs via `star run --seed --set` to its logged SHA-256; a Monte-Carlo regression layer gates ensemble statistics against frozen goldens at chi-square/Anderson–Darling 99 % bounds behind a two-key golden-update path (CI rejects a golden change not carried through the diff-summary tooling); `star_reacher.gym.SpaceEnv` is a ~200-line Gymnasium adapter that passes `check_env` with Gym-side seeding bit-identical to core seeding (the core carries zero Gym knowledge); and `star_reacher.ml.OnnxGnc` runs an ONNX MLP fitted in an external framework as an in-the-loop controller on onnxruntime CPU, closing the loop on x86-64. `star verify` wires the first two criteria as V028/V029, both demonstrated able to fail; the literal Pi 5 and x86-64-versus-aarch64 cross-platform clauses of the ONNX criterion are deferred, fully prepared, to the [pre-release checklist](docs/release_checklist.md) item 9 (PRD §9 valve) and are carried by Phase 9. Phase 6's audited gate gaps remain tracked as named remediation items — read the [Roadmap](#roadmap) and the PRD's Phase 6 entry before relying on any single Phase 6 criterion.
>
> On top of the Phase 1 skeleton (Apache-2.0 per [ADR 0001](docs/adr/0001-license-and-visibility.md)), the Phase 2 math kernel, the Phase 3 environment force models, and the Phase 4 vehicle 6DOF stack, every run is now inspectable, replayable, and exportable: `star plot` renders the full quicklook PNG set (groundtrack with embedded coastline through per-source force/torque budgets) with its plot-feeding arrays gated by golden vectors; `star view` writes a self-contained single-file HTML WebGL playback viewer (vendored three.js, zero network requests, measured decimation error bound); `star export` adds bit-exact NPZ and pandas-ready Parquet alongside CSV; the loader derives osculating elements; and two new example missions — Mission A (cislunar, the < 60 s Pi 5 performance-gate mission) and a heliocentric Mars-cruise arc on the new Sun central body — run with bit-identical reruns. Performance and dependency-minimality gates are live in CI; the evidence items that need hardware or software the maintainer does not currently have (a Pi 5, a MATLAB license) are fully prepared, registered on the [pre-release checklist](docs/release_checklist.md), and scheduled as Phase 9. Track what is actually built in the [Roadmap](#roadmap).
...
Nine independently shippable phases, each gated on red-team-checkable exit criteria (full detail in [`PRD.md` §8](PRD.md)). Honest status per phase:
...
| 8 | **Validation campaign, report, release** — full cross-tool table, completed report, fresh-machine walkthrough, tagged release | Every cross-tool case within stated tolerance (e.g., trans-lunar < 1 km at arrival); byte-identical doc rebuilds; fresh-machine README walkthrough succeeds; release wheels pass `verify --quick` on all four platforms | **Complete — released as v0.8.0** ([handoff](docs/release_handoff.md)). Deliverables complete and every locally-closable criterion met and verified: the scientific report and math library are complete (chapter-accretion lint green at 30 modules, all 199 code-cited equation labels and all 224 validation-table test IDs resolve) and build warning-clean via `star docs`, which scans each LaTeX log rather than trusting latexmk's exit code, and rebuild byte-identically after a full clean in the `docs` job; the report case-study figures regenerate bit-for-bit under the pinned matplotlib/FreeType; the fresh-machine [walkthrough](docs/walkthrough.md) reaches a rendered trajectory using README commands only and `star verify` passes (29/29) in ~9 s on x86-64; the example gallery is committed. The extended cross-tool table is complete: all five frozen-GMAT cases are measured and recorded, each inside its gate (Molniya 0.16 m, lunar orbiter 7.2 m, Mars orbiter 79.5 m, Earth–Mars cruise 0.95 km at the end of its committed 7-day arc, trans-lunar arrival 18.1 m against the 1 km tolerance; `tests/golden/crosstool/manifest.toml`), which discharges [pre-release checklist](docs/release_checklist.md) item 10. The cruise case's substituted comparison epoch — arc end rather than Mars-SOI arrival — is registered as checklist item 12, a residual rather than a release blocker. Criterion 4's `< 10 min on a Pi 5` timing clause stays deferred, fully prepared, to checklist item 1 (PRD §9 valve) and is carried by Phase 9 rather than by this release. Both gating items are discharged 2026-07-27: the green post-merge `nightly` run (checklist item 3) and the four-platform release wheels, which built and `verify --quick`-smoked in the tag-triggered `release` job (item 11) — see the [release handoff](docs/release_handoff.md) |
| 9 | **Hardware and external-tool validation** — the exit-criterion clauses deferred through the PRD §9 valve for want of a resource, plus the one deferred format field: the Pi 5 measurement campaign, the ONNX loop closure on aarch64, the MATLAB `parquetread` transcript, and the SRLOG 1.4 `gnc.error_layout` header field | On real Pi 5 silicon — quicklook PNGs render headless and the viewer opens in Pi 5 Chromium with zero network requests (Phase 5 criteria 1 and 2); Mission A < 60 s wall, ascent ≥ 100× real time open-loop and with the C++ GNC stack in the loop, SRLOG sustained write ≥ 50 MB/s (Phase 5 criterion 4, Phase 6 criterion 10); `star verify` prints `VERIFY: PASS` in < 10 min (Phase 8 criterion 4); the ONNX controller closes the loop there and its x86-64-versus-aarch64 final states agree within the D-10 1e-9 bound (Phase 7 criterion 4). Off it — exported Parquet loads in MATLAB with the documented schema and bit-exact values, transcript committed (Phase 5 criterion 3); `gnc.error_layout` is written and read at SRLOG 1.4, with a conformance test proving `star consistency` refuses a log whose declared error layout is not quaternion-led | Scheduled, not started — gated on Pi 5 silicon and a MATLAB-licensed host, neither available to the maintainer. Every clause keeps its [pre-release checklist](docs/release_checklist.md) entry (items 1, 2, 4, and 9) with what it carries, its prepared procedure, and its deferral history intact: moving a clause into this phase re-schedules it and neither discharges nor waives it. Nothing here is measured yet, and the nightly `ubuntu-24.04-arm` leg remains a documented aarch64 proxy that is never reported as a Pi 5 measurement |
...
That is the hardware floor and a binding performance gate, not an afterthought: a multi-day cislunar transfer targets under 60 s of wall time on a single Pi 5 core, and `star verify` targets the full acceptance suite in under 10 minutes on the same hardware. The literal Pi 5 wall-time numbers await Pi 5 silicon — no Pi 5 hardware is available to the maintainer yet, so those clauses are deferred, fully prepared, to the [pre-release checklist](docs/release_checklist.md) (item 1, procedure [`docs/perf/pi5_checklist.md`](docs/perf/pi5_checklist.md)), scheduled as [Phase 9](#roadmap), and the nightly `ubuntu-24.04-arm` leg gates them as a documented aarch64 proxy in the interim. For reference, the full `star verify` suite (29 checks) runs in about 9 seconds on the maintainer's x86-64 development host — well inside the 10-minute budget, though an x86-64 number is not a Pi 5 number and does not discharge the clause.
