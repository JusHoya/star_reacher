"""Regenerate the continuous cross-tool DE440 excerpt in this directory.

The Phase 2 excerpt (``excerpt_de440s.sreph``) holds isolated records around
discrete validation epochs and cannot feed a continuous multi-day propagation.
The Phase 3 cross-tool drag mission (``missions/leo_drag_hp.toml``) needs the
Sun direction from the ephemeris continuously over its 7-day window, so this
script cuts a second, CONTIGUOUS excerpt: for each needed segment (sun, emb,
earth, moon - the bodies that compose the geocentric Sun and Moon), the
...
``manifest.toml`` alongside the Phase 2 excerpt's.

Maintainer-side: requires the fetched kernels (``star data fetch de440s``,
network on first run). Regenerating is
``python tests/golden/ephemeris/generate_crosstool.py``; the output bytes are
a pure function of the pinned source kernels.
"""
...
OUT_NAME = "excerpt_de440s_crosstool.sreph"

# 2026-01-01T00:00:00 TDB (a TDB midnight, exact in binary64): 9496.5 days
# past J2000 (2000-01-01T12:00:00 TDB), i.e. 9496.5 * 86400 s. The mission
...
SEGMENTS_NEEDED = ("sun", "emb", "earth", "moon")
