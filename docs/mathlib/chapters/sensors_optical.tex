...
The planned gates, mirroring Phase~6 exit criteria~1 and~9:
\begin{itemize}
  \item \textbf{Aberration recomputation (exit criterion 9).} An
    independent NumPy implementation of
    equations~\eqref{eq:optical:beta}--\eqref{eq:optical:abmag},
    written from this chapter alone, reproduces the core's apparent
    directions to $< \qty{1}{\mas}$ at
    $\beta$ corresponding to \qty{30}{\kilo\metre\per\second}, across a
    grid of source--velocity angles including the extremes
    $\vartheta \in \{0, \pi/2, \pi\}$; the displacement is verified to
    be \emph{toward} the velocity and \qty{20.49}{\arcsecond} at
    $v = \qty{29.78}{\kilo\metre\per\second}$, $\vartheta = \pi/2$.
  \item \textbf{Star-tracker statistics (exit criterion 1).} The
    ensemble-mean statistic of
    equations~\eqref{eq:optical:extract}--\eqref{eq:optical:stbounds}
    over 1{,}000 seeded draws falls inside $[2.850, 3.154]$; per-axis
    sample variances match the configured sigmas within their own
    two-sided 95\,\% bounds; the same seeds reproduce the same
    statistic bit-for-bit.
  \item \textbf{Gating.} Deterministic geometry cases: boresight swept
    across each exclusion boundary and the rate swept across
    $\omega_{\max}$ flip the flag at the configured thresholds exactly;
    an umbra transit (Chapter~\ref{ch:srp} shadow) drops the sun-sensor
    flag; draws are consumed identically on both sides of every gate.
  \item \textbf{Sun-sensor statistics.} Tangent-plane ensemble statistic
    inside $[1.878, 2.126]$ over 1{,}000 draws.
  \item \textbf{Bit identity.} Two runs of the same seeded scenario
    produce byte-identical \texttt{sensors.st.*} and
    \texttt{sensors.sun.*} channels (exit criterion~1).
\end{itemize}
