...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.2cm}}
    ...
    \testid{test_aberration_matches_independent_reference} &
      ...
      ephemeris-backed scenario. Measured worst residual
      \qty{2.576e-8}{\mas} against the criterion's \qty{1}{\mas}; see the
      tolerance note below. \\
...
\qty{2.576e-8}{\mas} --- some $3.9\times10^{7}$ times inside the gate.
Setting the assertion at the requirement rather than near the achievable
agreement has a consequence worth stating plainly: a reference mutation
...
therefore survives. It survives because the tolerance is loose relative to
the achievable agreement, not because \qty{1}{\mas} is forced by anything
physical: at \qty{e-5}{\mas}, which would still leave roughly
$400\times$ headroom over the measured residual, the same mutation is
rejected by a factor of about $4.7\times10^{4}$. Tightening the assertion
is accordingly a
tolerance choice available at any time, and is tracked as
\texttt{KNOWN-ISSUE-P6-2}. As of this commit the committed assertion reads
\qty{1}{\mas}.
