...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.2cm}}
    ...
    \testid{gnc_cycle_double_run_is_byte_identical} &
      ...
      logs, covering the optical sensor channels. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Independent aberration recomputation (exit criterion 9).}
    A NumPy implementation of
    equations~\eqref{eq:optical:beta}--\eqref{eq:optical:abmag} written
    from this chapter alone, run against logged truth and ephemeris inputs
    from a committed scenario, gating agreement at $< \qty{1}{\mas}$. The
    comparison is first-order form against first-order form: this chapter
    declares equation~\eqref{eq:optical:aberration} normative, and the gap
    to the exact relativistic form is \qty{0.51}{\mas} at Earth's mean
    heliocentric speed and \qty{0.81}{\mas} once low-Earth-orbit speed is
    added, which would consume most of the \qty{1}{\mas} budget on a
    formula substitution the chapter does not ask for. The magnitude,
    direction, and angular law of the implemented formula are covered by
    the table above; what remains is the independent reimplementation and
    the in-loop scenario comparison.
