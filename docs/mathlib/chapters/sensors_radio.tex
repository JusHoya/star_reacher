...
consumed identically on both sides. Outstanding, recorded rather than
tabled: the enabled Gauss--Markov component of
equation~\eqref{eq:radio:gm} on the fix is implemented but is pinned only
by the shared derivation and by the IMU chapter's recursion test, not yet
by its own committed value-for-value case; and the altimeter's own
