...
\sin\theta\,\hat{\vv{h}}^{I})$, whose norm is $\abs{\cos\theta}$, so for
...
switching at $\abs{\cos\theta} = 10^{-6}$ --- two decades above the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.4cm}}
    ...
    \testid{vehicle6dof_pitch_program_triad_matches_definition_at_vertical} &
      ...
      cancellation remainder of norm $\abs{\cos\theta}$, so its rounding is
      amplified by $1/\abs{\cos\theta}$, while
