...
% references. The equation labels eq:twobody:accel and eq:twobody:rk4 are
% echoed verbatim in comments in cpp/src/models/twobody.cpp (FR-29
...
\end{equation}
using the classical fourth-order Runge--Kutta method (RK4) with a fixed step
$h = \Delta t$; see Hairer, N{\o}rsett, and Wanner~\cite{hairer1993} for the
method and its coefficients. One step from $\vv{y}_n$ to $\vv{y}_{n+1}$ is
\begin{equation}
  \begin{aligned}
    \vv{k}_1 &= \vv{f}(\vv{y}_n), \\
    \vv{k}_2 &= \vv{f}\!\bigl(\vv{y}_n + \tfrac{h}{2}\,\vv{k}_1\bigr), \\
    \vv{k}_3 &= \vv{f}\!\bigl(\vv{y}_n + \tfrac{h}{2}\,\vv{k}_2\bigr), \\
    \vv{k}_4 &= \vv{f}\!\bigl(\vv{y}_n + h\,\vv{k}_3\bigr), \\
    \vv{y}_{n+1} &= \vv{y}_n
      + \tfrac{h}{6}\,\bigl(\vv{k}_1 + 2\vv{k}_2 + 2\vv{k}_3 + \vv{k}_4\bigr),
  \end{aligned}
  \label{eq:twobody:rk4}
\end{equation}
with local truncation error $\mathcal{O}(h^{5})$ and global error
$\mathcal{O}(h^{4})$~\cite{hairer1993}. Because the system is autonomous, no
explicit time argument appears in the stage evaluations.
...
\begin{enumerate}
  \item \textbf{Module and traceability.} The model lives in
    ...
    \texttt{eq:twobody:rk4} (the RK4 stage update) verbatim, so each code
    block traces to its defining equation and the chapter-manifest lint can
    hold code and document together.
