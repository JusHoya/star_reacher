...
\texttt{truth} group (exit criterion~7). The constants
$\quat{q}_{b2c}$, $\vv{r}_{\mathrm{cam}}^{B}$, and the intrinsics are
\emph{not} carried in the log: a consumer reconstructing
equation~\eqref{eq:camera:pose} reads them from the resolved
configuration, which is where \texttt{docs/formats/srlog\_v1.md} places
them. An earlier revision of this chapter stated that they were echoed
once in the log header; that echo was never implemented, and verifying
against a run confirms no intrinsic, extrinsic, or landmark value appears
anywhere in the header. Adding it would be a log-format change. This is
the direct cause of the uncovered intrinsics clause recorded after the
evidence table.
...
\begin{itemize}
  \item \textbf{Intrinsics clause of exit criterion~7.} The criterion
    ...
    merely untabled. The gap is a specification one and predates the tests:
    Section~\ref{sec:camera:pose} previously asserted that the intrinsics
    and extrinsics were echoed once in the log header, and no such echo was
    ever implemented --- verified against a run, no intrinsic, extrinsic,
    or landmark value appears anywhere in the header. Closing the clause
    requires either implementing that echo and gating it, which is a
    log-format change, or amending the criterion to drop the clause on the
    recorded ground that intrinsics are configuration rather than a logged
    channel. The intrinsics themselves
