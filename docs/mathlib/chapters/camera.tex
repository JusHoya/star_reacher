...
The planned gates, mirroring Phase~6 exit criterion~7 (and~9 for the
shared aberration path):
\begin{itemize}
  \item \textbf{Bit-exact pose and intrinsics (exit criterion 7).} The
    \texttt{sensors.cam} pose channels compare byte-identical to the
    \texttt{truth} channels of the same run; intrinsics and extrinsics
    compare byte-identical to the resolved configuration.
  \item \textbf{Independent pinhole recomputation (exit criterion 7).}
    A NumPy script written from this chapter alone --- truth channels
    and resolved config in, equations~\eqref{eq:camera:pose},
    \eqref{eq:camera:landmark}--\eqref{eq:camera:proj} out --- matches
    every logged visible-landmark $(u, v)$ to $< 10^{-6}$ pixels
    across a scenario exercising off-axis mounts
    ($\vv{r}_{\mathrm{cam}}^{B} \ne \vv{0}$,
    $\quat{q}_{b2c} \ne$ identity) and anisotropic focal lengths
    ($f_x \ne f_y$).
  \item \textbf{Visibility truth table.} Constructed geometries drive
    each of the four tests across its boundary independently ---
    behind-camera, each sensor edge at the half-pixel convention, limb
    grazing via equation~\eqref{eq:camera:nearside}, and a third-body
    transit via equation~\eqref{eq:camera:occlusion} --- and the flag
    flips exactly at the stated thresholds.
  \item \textbf{Aberration consistency (exit criterion 9).} On a
    common scenario the camera Sun vector and the sun-sensor
    noise-free direction agree to rounding, both matching the
    independent aberration recomputation of
    Chapter~\ref{ch:sensors-optical} to $< \qty{1}{\mas}$.
  \item \textbf{Bit identity.} Two runs of the same scenario produce
    byte-identical \texttt{sensors.cam.*} channels.
\end{itemize}
