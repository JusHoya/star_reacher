...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.2cm}}
    ...
    \testid{gnc_cycle_double_run_is_byte_identical} &
      ...
      logs, covering the camera channels. The hook consumes no draws, so
      its contribution is deterministic by construction. \\
    \bottomrule
  \end{tabular}
\end{table}

Exit criterion~7's bit-exactness clause is satisfied \emph{by
...
exists that could differ. The remaining criterion-7 clause --- the
independent NumPy recomputation of
equations~\eqref{eq:camera:pose} and
\eqref{eq:camera:landmark}--\eqref{eq:camera:proj} matching every logged
visible-landmark $(u, v)$ to $< 10^{-6}$ pixels on a scenario with an
off-axis mount and $f_x \ne f_y$ --- is outstanding, as is the
aberration-consistency check pairing the camera Sun vector against the
sun-sensor direction.
...
implementation deliberately does not make. Exit criterion~7 gates the pose,
intrinsics, and landmark pixels, all of which the current layout carries.
