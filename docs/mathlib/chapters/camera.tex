...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.2cm}}
    ...
    \testid{test_landmark_pixels_match_independent_reference} &
      ...
      hide. The principal point is off-axis \emph{as of} the fixture
      revision recorded with
      \testid{test_camera_echo_fixture_is_not_degenerate}: it previously
      sat at $(511.5, 383.5)$, which under the pixel-center convention is
      exactly the image center $((W-1)/2, (H-1)/2)$ of the
    ...
    \testid{test_camera_intrinsics_echo_reproduces_logged_pixels} &
      ...
      from another; a companion assertion pins those four properties so
      the fixture cannot silently become degenerate. \\
    \testid{test_camera_echo_mutation_is_detected} &
      Non-vacuity of the row above, by construction rather than by
      inspection: the test rewrites the echoed $f_x$ in the emitted
      header bytes and requires the reconstruction to move by more than
      the gate tolerance, so a gate that ignored the echo could not
      pass. \\
    \bottomrule
  \end{tabular}
\end{table}
