...
\begin{enumerate}
  ...
  \item \textbf{Leap-second knowledge horizon.} The bundled table encodes
    ...
    IERS Bulletin~C~70 (July 2025), which announced that no leap second is
    introduced at the end of December 2025~\cite{iersbc70}. The first UTC
    date at which a leap second \emph{not} present in the table could take
    effect is therefore 2026-07-01 (the end-of-June-2026 insertion
    opportunity, which Bulletin~C~70 does not rule out).
...
accuracy through 2026-06-30; TDB$-$TT within the D-6 budget of
...
\begin{itemize}
  ...
  \item Calendar input on or after the table expiry (2026-07-01) is
...
Bulletin~C~\cite{iersbc70}. Terrestrial Time is defined with a constant
...
\begin{enumerate}
  ...
  \item \textbf{Leap-second table.} The 28-entry TAI$-$UTC step history is
    ...
    series, verified through Bulletin~C~70~\cite{iersbc70}), scanned with a
    fixed-order loop (branch-stable, D-10). The table version string and
    expiry date (2026-07-01) are exposed programmatically
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.2cm}}
    ...
    \testid{time_leap_table_golden} &
      ...
      history; expiry metadata (2026-07-01); post-expiry lookups return
      \qty{37}{\second}. \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{6.6cm}}
    ...
    \testid{test_time_epoch_expiry_warning} &
      Mission epochs on or after 2026-07-01 (by UTC calendar date,
...
analytical TDB$-$TT model; on the IERS Bulletin~C series~\cite{iersbc70}
