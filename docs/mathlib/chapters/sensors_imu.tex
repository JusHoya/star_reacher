...
\texttt{cpp/src/sensors/imu.cpp}. The full error chain of
equations~\eqref{eq:imu:gyro}--\eqref{eq:imu:quant} and the Allan
identification of Section~\ref{sec:imu:recovery} have landed, so every gate
this chapter specifies now appears in the tables below with a
machine-checked test identifier. The zero-error increments remain the
model's zero-coefficient special case: each error stage degenerates to an
exact identity, which is itself asserted rather than assumed.

% longtable rather than table: these rows outgrew a single page once every
% stage of the error chain earned a row of its own beside the exit-criterion-1
% bit-identity row, and a float too large for the page drops its trailing rows
% from the PDF while the source still lists them.
\begin{longtable}{lp{8.4cm}}
  \caption{Validation evidence for the IMU: core (doctest,
    \texttt{cpp/tests/test\_sensors.cpp},
    \texttt{cpp/tests/test\_gnc\_cycle.cpp}).}
  \label{tab:imu:validation} \\
  \toprule
  ...
  \bottomrule
  \endlastfoot
    \testid{sensors_ideal_imu_trapezoidal_accumulation_and_reset} &
      Two cycles of held endpoint kinematics, with $\Delta t$ and endpoint
      values chosen exactly representable in binary64, accumulate to the
      trapezoidal sums of equation~\eqref{eq:imu:quadrature} under bit
      equality (not a tolerance); \texttt{sample()} then resets the
      accumulators, so the following interval integrates only its own
      cycle. \\
    \testid{sensors_imu_zero_error_config_is_the_ideal_special_case} &
      Two instances with all-zero coefficients and \emph{different} seeds
      emit bit-identical increments, each equal to the raw
      quadrature~\eqref{eq:imu:quadrature} --- the zero-error special case
      is an exact identity through every stage of
      equations~\eqref{eq:imu:gyro}--\eqref{eq:imu:quant}, not an
      approximation. \\
    \testid{sensors_imu_turnon_bias_and_draw_schedule} &
      With only the turn-on bias~\eqref{eq:imu:turnon} enabled, the emitted
      increment equals $\Delta\vv{\theta} + \vv{b}_{g0}\Delta t_s$ under bit
      equality, with $\vv{b}_{g0}$ taken from an independently replicated
      \texttt{sensors.imu} stream --- which pins the normative
      initialization draw schedule, not merely the arithmetic. \\
    \testid{sensors_imu_scale_factor_and_misalignment} &
      The $\mat{M} = \mat{S} + \mat{\Gamma}$ assembly of
      equation~\eqref{eq:imu:mis} places scale factors (configured in ppm)
      on the diagonal and the six misalignment entries off it in the order
      $xy, xz, yx, yz, zx, zy$; the distorted increment matches
      $(\mat{I} + \mat{M})\Delta\vv{\theta}$ to $10^{-15}$ relative. An
      unknown key, a wrong-length vector, and a negative magnitude are each
      refused by \texttt{std::invalid\_argument}. \\
    \testid{sensors_imu_quantizer_carry_and_ties} &
      Equation~\eqref{eq:imu:quant} on exact binary fractions, asserted with
      bit equality: a truth increment of exactly half a quantum emits the
      alternating sequence $q, 0, q, 0, \dots$, exercising both the
      round-half-up tie (toward $+\infty$ by construction) and the residual
      carry. The accumulated output tracks the accumulated input to within
      half a quantum at every sample and closes exactly over a whole number
      of quanta --- quantization is lossless in accumulation. \\
    \testid{sensors_imu_gauss_markov_recursion_and_preset_map} &
      The in-run bias follows recursion~\eqref{eq:imu:gm} value-for-value
      against an independently replicated stream, including its stationary
      initialization. The preset map~\eqref{eq:imu:presetmap} is checked
      against the closed form~\eqref{eq:imu:gmadev}: the ADEV at the
      \eqref{eq:imu:gmpeak} abscissa $\tau^{*} = 1.8926\,\tau_c$ equals
      $0.664282\,B$ to $10^{-4}$ relative, and $\tau^{*}$ is confirmed to be
      a maximum by evaluation on both sides. \\
    \testid{sensors_factory_kind_vocabulary} &
      The factory constructs every canonical FR-23 kind at its configured
      rate, each reporting its own group name, and rejects both a
      non-canonical kind name and a zero sample rate by
      \texttt{std::invalid\_argument} --- refusal by name rather than a
      stub, so a typo cannot silently enter a run. \\
    \testid{gnc_cycle_double_run_is_byte_identical} &
      Bit identity at the whole-file level: two runs of the same seeded GNC
      scenario on the same binary produce byte-identical logs. The scenario
      declares exactly one sensor --- an ideal IMU at \qty{10}{\hertz} with
      no noise coefficients --- so this row covers the noise-free increment
      path and the writer, not the stochastic path. Exit criterion~1's
      every-sensor clause is carried by
      \testid{test_full_sensor_suite_reruns_bit_identical} in
      Table~\ref{tab:imu:validation-py}. \\
\end{longtable}

\begin{table}[htbp]
  \centering
  \caption{Validation evidence for the Allan identification of
    Section~\ref{sec:imu:recovery} (exit criterion~1).}
  \label{tab:imu:validation-allan}
  \begin{tabular}{lp{8.4cm}}
    \toprule
    Test ID & Acceptance basis \\
    \midrule
    \testid{sensors_imu_allan_recovers_arw_and_bias_instability} &
      The Section~\ref{sec:imu:recovery} procedure on a
      \qty{e4}{\second} static record ($\vv{\omega}^{B} \equiv \vv{0}$,
      $\vv{f}^{B} \equiv \vv{0}$, fixed seed), applied per instrument:
      the overlapping estimator~\eqref{eq:imu:oadev} at the one-second
      anchor with the known Gauss--Markov term removed in quadrature
      \eqref{eq:imu:recovery} returns $N$ to $0.8\,\%$ (gyro) and
      $0.08\,\%$ (accelerometer), and the peak anchor with the recovered
      white term removed~\eqref{eq:imu:recoverybi} returns $B$ to
      $2.7\,\%$ and $3.7\,\%$ --- all inside the $\pm 10\,\%$ gate. The
      analytic curve $\sqrt{N^2/\tau + \sigma_{\mathrm{GM}}^2(\tau)}$
      overlays the estimate across the octave grid within four times the
      cluster-count scatter. \\
    \bottomrule
  \end{tabular}
\end{table}

% longtable rather than table: these rows outgrew a single page once the
% stochastic-path identity rows landed, and a float too large for the page
% silently drops its trailing rows rather than warning usefully.
\begin{longtable}{lp{7.2cm}}
  \caption{Validation evidence for the IMU: in-loop increments and
    run-to-run identity (pytest,
    \texttt{tests/python/test\_gnc\_missions.py}).}
  \label{tab:imu:validation-py} \\
  \toprule
  Test ID & Acceptance basis \\
  \midrule
  \endfirsthead
  ...
  \bottomrule
  \endlastfoot
    \testid{test_ideal_imu_increments_bit_exact} &
      On a logged free-flyer run the emitted $\Delta\vv{\theta}$ equals the
      quadrature~\eqref{eq:imu:quadrature} of the logged truth rate over
      the preceding cycle under array bit equality, and $\Delta\vv{v}$ is
      exactly zero --- an accelerometer in free fall reads zero
      (equation~\eqref{eq:imu:specificforce}). \\
    \testid{test_full_sensor_suite_reruns_bit_identical} &
      Exit criterion~1's every-sensor clause. Two seeded runs of the
      ...
      equality across every one of the six \texttt{sensors.*} groups, so
      two groups differing in compensating ways could not hide behind the
      file hash. The \texttt{sensors.imu} group is included; that mission
      declares the IMU with a sample rate alone, so the identity it
      establishes for this chapter is again of the noise-free path. \\
    \testid{test_stochastic_imu_reruns_bit_identical} &
      Exit criterion~1's every-sensor clause for the \emph{stochastic}
      path, which the row above does not reach. The same six-sensor
      mission is run twice with the IMU's full error chain active ---
      turn-on bias, Gauss-Markov bias instability, angle and velocity
      random walk, quantization, scale factor, and misalignment --- and
      compared by whole-file SHA-256 and then channel by channel. The
      quantizer carries a residual from each sample into the next
      (equation~\eqref{eq:imu:quant}), so a draw schedule that diverged at
      any one sample would contaminate every later sample rather than a
      single value; agreement across the whole record is therefore
      evidence about the schedule and not only about the outputs.
      Measured: seeding the IMU stream non-deterministically fails this
      row while leaving
      \testid{test_full_sensor_suite_reruns_bit_identical} green, which is
      the coverage gap it exists to close. \\
    \testid{test_stochastic_imu_identity_is_not_degenerate} &
      Non-vacuity of the row above. A build that dropped every IMU noise
      coefficient would satisfy a bit-identity assertion perfectly, so
      three runs that must \emph{differ} pin the fixture: a changed master
      seed (measured, \qty{1.68e-5}{\radian} worst increment difference),
      the quantizer step removed (\qty{9.86e-8}{\radian}), and the ideal
      IMU of the row above (\qty{1.26e-5}{\radian}). A fourth assertion
      requires the increment stream to take many distinct values, so a
      stream drawn once and held could not pass. \\
\end{longtable}

Three items specified by this chapter remain outstanding and are recorded
here rather than tabled, so the evidence tables state only evidence that
exists:
\begin{itemize}
  \item \textbf{Bit identity of the stochastic path (exit criterion 1).}
    Every committed bit-identity gate that includes an IMU declares it with
    a sample rate alone. The angle-random-walk draws, the Gauss--Markov
    recursion~\eqref{eq:imu:gm}, and the quantizer
    carry~\eqref{eq:imu:quant} therefore sit under no run-to-run identity
    assertion, and the quantizer carry is the only sensor state in this
    chapter that persists between samples --- exactly the state a
    reproducibility defect would live in. Closing this needs
    \texttt{random\_walk}, \texttt{bias\_instability}, \texttt{bias\_tau\_s},
    and a nonzero quantizer step added to a bit-identity mission, with the
    gate shown to go red when the Gauss--Markov state initialization is
    perturbed. The stochastic terms are covered value-for-value by
    \testid{sensors_imu_gauss_markov_recursion_and_preset_map} and by the
    quantizer row of Table~\ref{tab:imu:validation}; what is absent is the
    identity clause specifically.
  \item \textbf{Gate-preset design (exit criterion 1).} The dispersion
    argument closing Section~\ref{sec:imu:recovery} accounts for the ADEV
    estimator scatter but not for the amplification that
    equation~\eqref{eq:imu:recoverybi} introduces by subtracting the
    white-noise term before inverting~\eqref{eq:imu:bi}. Writing
    $A = \hat{\sigma}^2(\tau^{*})$, $W = \hat{N}^2/\tau^{*}$, and
    $G = A - W$, the relative error propagates as $\delta B/B =
    (A/G)\,(\delta\hat{\sigma}/\hat{\sigma})$, so a preset whose
    Gauss--Markov term is a minority of the Allan variance at $\tau^{*}$
    produces an unreliable gate: at $N = 0.1\ \mathrm{deg}/\sqrt{\mathrm{h}}$
    and $\tau_c = \qty{20}{\second}$, $G/W$ is only $0.46$ at
    $B = 1\ \mathrm{deg/h}$. The committed gate therefore takes
    $B = 4\ \mathrm{deg/h}$ ($G/W = 7.4$ by design, $7.96$ and $15.47$ as
    measured), and the design rule is $B \ge \sqrt{G/W}\,N/(0.664282
    \sqrt{1.8926\,\tau_c})$. This paragraph, not the estimator-scatter
    sentence above it, is the binding guidance for choosing a gate preset.
  \item \textbf{Coning preservation.} On a pure coning trajectory
    (Chapter~\ref{ch:rigidbody}, closed form), accumulated increments
    should match the analytic integral of the body rate to the quadrature
    bound of equation~\eqref{eq:imu:quaderr} and be measurably distinct
    from point-sampled $\vv{\omega}(t_k)\Delta t_s$. The increment
    interface and its quadrature are covered by the tables above; the
    closed-form coning case is not yet implemented.
\end{itemize}

A note on equation~\eqref{eq:imu:quantadev}, for anyone writing a
quantization-specific Allan gate: the $\sigma_Q(\tau) = q/(2\tau)$
signature assumes the carried residual is white sample to sample, which the
carry of equation~\eqref{eq:imu:quant} makes true only when the input is
dithered at or above roughly one quantum. Below that the residual is
correlated over $\sim q/\sigma_{\mathrm{in}}$ samples and the measured ADEV
falls below this formula at short cluster times --- a property of the
model, not a defect. The committed Allan gate runs with the quantizer
disabled, so the term is exactly zero there.
