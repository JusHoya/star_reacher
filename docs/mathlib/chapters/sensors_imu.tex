...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.4cm}}
    ...
    \testid{gnc_cycle_double_run_is_byte_identical} &
      Exit criterion~1 bit identity at the whole-file level: two runs of
      the same seeded GNC scenario on the same binary produce
      byte-identical logs, covering the sensor channels. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{Validation evidence for the ideal IMU: in-loop increments
...
Two items specified by this chapter remain outstanding and are recorded
