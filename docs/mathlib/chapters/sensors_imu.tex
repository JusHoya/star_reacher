...
The module implementing this chapter has landed in part. The zero-error
special case of the measurement model --- the increment interface, its
trapezoidal quadrature~\eqref{eq:imu:quadrature}, and the sample-and-reset
semantics --- is implemented in \texttt{cpp/src/sensors/imu\_ideal.cpp} and
is covered by the tables below. The error chain of
equations~\eqref{eq:imu:gyro}--\eqref{eq:imu:quant} and the Allan machinery
of Section~\ref{sec:imu:recovery} are \emph{not} yet implemented; their
gates remain a plan and are listed separately at the end of this section,
so the tables state only evidence that exists.

\begin{table}[htbp]
  \centering
  \caption{Validation evidence for the ideal (zero-error) IMU: core
    (doctest, \texttt{cpp/tests/test\_sensors.cpp},
  ...
  \begin{tabular}{lp{8.4cm}}
    ...
    \testid{sensors_factory_kind_vocabulary} &
      The factory constructs the \texttt{imu} kind at its configured rate
      and rejects both an unimplemented sensor kind and a zero sample rate
      by \texttt{std::invalid\_argument} --- refusal by name rather than a
      stub, so an unimplemented FR-23 kind cannot silently enter a run. \\
...
\begin{table}[htbp]
  \centering
  \caption{Validation evidence for the ideal (zero-error) IMU: in-loop
    increments (pytest, \texttt{tests/python/}).}
...
The gates below specify the error-chain model and remain outstanding until
its module lands; they mirror Phase~6 exit criterion~1:
\begin{itemize}
